...
describe('cloneRepo', () => {
  ...
  it('should clone a repository with a valid GitHub URL', async () => {
    ...
    expect(fs.mkdirSync).toHaveBeenCalledWith(expectedRepoPath, { recursive: true });
...
describe('removeRepo', () => {
  ...
  it('should remove the repository when the path is valid and exists', async () => {
    ...
    expect(fs.rm).toHaveBeenCalledWith(repoPath, { recursive: true }, expect.any(Function));
  });
  ...
  it('should throw an error when the repository does not exist', async () => {
    ...
    // Act & Assert
    await expect(removeRepo(repoPath)).rejects.toThrow('Repository does not exist');
    expect(fs.existsSync).toHaveBeenCalledWith(repoPath);
  });
  ...
  it('should prevent removal outside the project directory', async () => {
    ...
    // Act & Assert
    await expect(removeRepo(repoPath)).rejects.toThrow('Cannot remove files outside the project directory');
  });
  ...
  it('should prevent removal of the project directory itself', async () => {
    ...
    // Act & Assert
    await expect(removeRepo(repoPath)).rejects.toThrow('Cannot remove the project directory');
  });
  ...
  it('should handle errors during repository removal', async () => {
    ...
    // Act & Assert
    await expect(removeRepo(repoPath)).rejects.toThrow('Error removing the repository');
    expect(fs.rm).toHaveBeenCalledWith(repoPath, { recursive: true }, expect.any(Function));
  });
});
