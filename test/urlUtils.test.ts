import { extractOwnerAndRepo, checkUrlType, convertSshToHttps, extractPackageNameFromUrl, processUrl, UrlType } from '../src/utils/urlUtils';
...
describe('urlUtils', () => {
  describe('extractOwnerAndRepo', () => {
    ...
    it('should exit with return code 1 for invalid owner and repo', async () =>{
      ...
      await expect(() => extractOwnerAndRepo(gitHubUrl)).toThrow('process.exit: 1');
  ...
  describe('extractPackageNameFromUrl', () => {
    ...
    it('should exit with rc1 for invalid URL', async () => {
      ...
      await expect(() => extractPackageNameFromUrl(url)).toThrow('process.exit: 1');
      expect(exitSpy).toHaveBeenCalledWith(1);
      exitSpy.mockRestore();
    });
  });
  ...
  describe('processUrl', () => {
    ...
    it('should process an npm URL and return owner and repo', async () => {
      const url = 'https://www.npmjs.com/package/package-name';
      (getGitHubRepoFromNpmUrl as jest.Mock).mockResolvedValue('git@github.com:owner/repo.git');
    ...
    await expect(processUrl(UrlType.Invalid, url)).rejects.toThrow('process.exit: 1');
    expect(exitSpy).toHaveBeenCalledWith(1);
    exitSpy.mockRestore();
  });
  });
