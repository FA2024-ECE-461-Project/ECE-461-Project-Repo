...
describe('cli', () => {
  ...
  it('should log error and exit if file cannot be read', async () => {
    ...
    (readUrlsFromFile as jest.Mock).mockRejectedValue(new Error('File read error'));

    await cli();

    expect(log.error).toHaveBeenCalledWith('Unable to read URLs from file test.txt', expect.any(Error));
    expect(process.exit).toHaveBeenCalledWith(1);
  });

  it('should log error and exit if URL processing fails', async () => {
    process.argv = ['node', 'cli.js', 'test.txt'];
    (readUrlsFromFile as jest.Mock).mockResolvedValue(['https://github.com/user/repo']);
    (checkUrlType as jest.Mock).mockReturnValue('github');
    (processUrl as jest.Mock).mockRejectedValue(new Error('URL processing error'));

    await cli();

    expect(log.error).toHaveBeenCalledWith('Error processing URL https://github.com/user/repo:', expect.any(Error));
    expect(process.exit).toHaveBeenCalledWith(1);
  });
  ...
  it('should process URLs and log metrics', async () => {
    ...
    (readUrlsFromFile as jest.Mock).mockResolvedValue(['https://github.com/user/repo']);
    (checkUrlType as jest.Mock).mockReturnValue('github');
    (processUrl as jest.Mock).mockResolvedValue({ owner: 'user', repo: 'repo' });
    ...
    expect(log.info).toHaveBeenCalledWith('Processing URL: https://github.com/user/repo');
    expect(console.log).toHaveBeenCalledWith(JSON.stringify(mockMetrics, null, 2));
  });
});