...
async function __findFolder(
  ...
  async function walkDir(currentPath: string): Promise<string | null> {
    const files = await fs.promises.readdir(currentPath, {
      withFileTypes: true,
    }); // get a list of files and directories specified by currentPath
    let keywords =
      folderType === "test"
    ...
    if (folderType !== "test" && folderType !== "src") {
      console.error(
        "Invalid folder type: only test or src folder types are allowed",
      );
      return null;
    }
    for (const file of files) {
      ...
      if (file.isDirectory()) {
        //only look for keywords in the tuple, can be improved
        if (keywords.includes(file.name)) {
          return fullPath;
        }
        const result = await walkDir(fullPath);
        if (result) {
          // when result is a valid path with name not in the tuple
          return result;
        }
      }
    }
...
async function _getCoverageScore(clonedPath: string): Promise<number> {
  ...
  const [numSrc, numTest, hasIntegration]: [number, number, number] =
    await Promise.all([
