...
export function calculateResponsiveness(metrics: RepoDetails): number {
  ...
  if (
    ...
    const timeDifferenceIssues =
      currentDate.getTime() - startDateIssues.getTime();
    const weeksDifferenceIssues = Math.ceil(
      timeDifferenceIssues / millisecondsInAWeek,
    );
  ...
  const timeDifferenceCommits =
    currentDate.getTime() - startDateCommits.getTime();
  const weeksDifferenceCommits = Math.ceil(
    timeDifferenceCommits / millisecondsInAWeek,
  );
